...
- **Notes**: 需要考虑各种边缘情况，如字段别名、表别名等

## [ ] 任务3: 修改FieldCheckAgent，集成新的定位逻辑
- **Priority**: P1
- **Depends On**: 任务2
- **Description**: 
...
- **Test Requirements**:
  - `programmatic` TR-5.1: 测试1000行以上SQL文件的解析时间
  - `programmatic` TR-5.2: 测试不同格式SQL文件的解析成功率
...
- **Test Requirements**:
  - `programmatic` TR-6.1: 端到端测试，验证完整流程的正确性
  - `human-judgment` TR-6.2: 验证系统的整体用户体验
